...
async def evaluate_agent_performance(
    criteria: list[str], result: AgentRunResult, output_transformer: callable = None
) -> JudgeFeedback:
...
async def test_agent_no_legal_domain():
    ...
    judge_feedback = await evaluate_agent_performance(
        criteria, result, output_transformer=lambda output: output.format_article()
    )
    ...
    for criterion in judge_feedback.criteria:
        assert criterion.passed, (
            f"Criterion failed: {criterion.criterion_description}, {criterion.judgement}"
        )
