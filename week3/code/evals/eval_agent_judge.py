...
async def evaluate_single_result(
    row: dict, judge: Agent, file_index: dict[str, str], instructions: str
) -> tuple[dict, any]:
    """
...
async def run_complete_judge_evaluation(
    input_path: str,
    model: str = "gpt-5-nano",
    max_concurrency: int = 10,
    output_path: str = None,
) -> tuple[CostInfo, pd.DataFrame, pd.Series, str]:
    """
    ...
    results = await run_judge_evaluation(
        rows, judge, file_index, search_agent.search_instructions, max_concurrency
    )
