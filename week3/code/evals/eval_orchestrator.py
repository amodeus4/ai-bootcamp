...
def main_cli():
    ...
    parser.add_argument(
        "--agent-model", default="gpt-4o-mini", help="Model to use for the agent"
    )
    parser.add_argument(
        "--judge-model", default="gpt-5-nano", help="Model to use for the judge"
    )
    parser.add_argument(
        "--concurrency", type=int, default=10, help="Maximum concurrent operations"
    )
    parser.add_argument(
        "--output", help="Output path for run results (auto-generated if not specified)"
    )
    ...
    report_path = (
        reports_dir
        / f"eval-report-{results['timestamp'].strftime('%Y-%m-%d-%H-%M')}.txt"
    )
